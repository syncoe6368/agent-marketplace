import { Sparkles, Eye, Star, TrendingUp } from 'lucide-react';
...
export default async function TrendingPage() {
  ...
  // Fetch agents — try weekly_views first, fall back to views_count
  ...
  const allAgents = (weeklyData || []) as AgentRow[];

  // Check if weekly_views has any data — if all zeros, re-sort by views_count
  const hasWeeklyData = allAgents.some(a => (a.weekly_views || 0) > 0);
  const sortedAgents = hasWeeklyData
    ? allAgents
    : [...allAgents].sort((a, b) => b.views_count - a.views_count);
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold">Trending Agents</h1>
        </div>
        <p className="text-muted-foreground text-lg">
          {hasWeeklyData
            ? 'Most viewed agents this week — see what the community is loving.'
            : 'Most popular agents on AgentHub — weekly tracking begins with your next visit!'}
        </p>
      </div>
              ...
              <Card className="group h-full hover:shadow-lg hover:border-primary/30 transition-all duration-200 relative overflow-hidden">
                {i < 3 && (
                  <div className="absolute top-0 right-0">
                    <div className={`text-white text-xs font-bold px-2 py-0.5 rounded-bl-lg ${rankColors[i]}`}>
                      #{i + 1}
                    </div>
                  </div>
                )}
                <CardContent className="p-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${categoryGradients[agent.category?.[0]?.slug || ''] || 'from-primary to-primary/70'} flex items-center justify-center text-white font-bold text-lg shrink-0`}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground truncate">
                        {agent.category?.[0]?.name || 'Uncategorized'}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                      {hasWeeklyData ? (
                        <>
                          ...
                          {(agent.weekly_views || 0).toLocaleString()} this week
                        </>
                      ) : (
                        <>
